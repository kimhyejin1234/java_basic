...
public class MyBirth {
	...
	/*
	 * -정보 은익은 데이터에 접근 제한을 걸어서 정보를 보호는 것이 목적이지만
	 * private 을 성정 했더니 데이터 접근 자체가 불가능해집니다.
	 * 
    ...
    */
	...
	public void setMonth(int month) {
		if(month <1 || month > 12) {
			System.out.println("잘못 입력된 month 입니다.");
		} else {
			this.month = month;
		}
	}
	...
	public void setDay(int day) {
		if(day > 31 || day <1 ) {
			System.out.println("잘못 입력된 day 입니다.");
		} else {
			if(this.month == 0) {
				System.out.println("일자를 입력하지 위해서는 월 입력이 선행되어야 합니다.!");
				return;
			}
	...
	public void birthInfo() {
		...
		if(this.year == 0 ||  this.month == 0 || this.day == 0 ) {
			System.out.println("날짜 필드중 초기화 되지 않은 데이타가 있습니다.");
			return;
		} 
	 ...
	 * 데이터와 그 데이터를 다루는 로직으로 하나로 묶어 쉽게 관리하고 유지 보수하기 위함 입니다.
	 */
